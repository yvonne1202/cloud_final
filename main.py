...
def editProfile():
	...
	conn = mysql.connector.connect(
		...
		database='cloudFinalDB')
	cur = conn.cursor()
	cur.execute("SELECT userId, email, password, name, phone FROM users WHERE email = ?", (session['email'], ))
...
def updateProfile():
	if request.method == 'POST':
		...
		try:
			cur = con.cursor()
			cur.execute('UPDATE users SET name = ?, phone = ? WHERE email = ?', (name, phone, email))
...
def register():
	if request.method == 'POST':
		...
		try:
			cur = con.cursor()
			cur.execute('INSERT INTO users (email, password, name, phone) VALUES (?, ?, ?, ?)', (email, password, name, phone))
